...
profile -> analyze -> improve -> re-experiment loop retrieves the relevant playbook remotely instead
of relying on local skills. Runs on a CPU Pod (no accelerator, no mount); reached over the same
streamable-http + kubectl port-forward pattern as the analysis MCP.
"""
...
from .library import KnowledgeLibrary, default_root
...
def build_server(library: KnowledgeLibrary, port: int = 8080) -> Any:
    ...
    @mcp.tool()
    def search_knowledge(query: str, chip: str = "") -> dict:
        ...
        hits = library.search(query, chip or None)
        return {"query": query, "chip": chip or "all", "count": len(hits), "results": [
